...
function timeSpanToString(startDate, endDate) {
  ...
  const sss = (time % 1000).toString().padStart(3, '0');
  const ss = (((time - sss) / 1000) % 60).toString().padStart(2, '0');
  const hh = ((Math.floor(time / 3600000))).toString().padStart(2, '0');
  const mm = (Math.floor((time % 3600000) / 60000)).toString().padStart(2, '0');
  return `${`${hh}:${mm}:${ss}.${sss}`}`;
}
